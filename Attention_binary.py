...
# 続く処理
x = LSTM(50, activation='relu')(attention)
outputs = Dense(1, activation='sigmoid')(x)  # 出力層の活性化関数を'sigmoid'に
...
history=model.fit(X_train, y_train, validation_data=(X_validation, y_validation), epochs=100, batch_size=32, verbose=1, callbacks=[early_stop])

# 最良のval_accuracyの値を取得
best_val_accuracy = max(history.history['val_accuracy'])
print(f"The best val_accuracy is {best_val_accuracy}")

# Make predictions
y_train_pred = model.predict(X_train)
y_validation_pred = model.predict(X_validation)

# y_train_predとy_validation_predを1次元配列にする
y_train_pred_flat = y_train_pred.flatten()
y_validation_pred_flat = y_validation_pred.flatten()

# 既存のdfからTrainとValidationのDateを取得
train_dates = train.index[n_steps:]
val_dates = val.index[n_steps:]

# TrainのDateとPredicted valueを組み合わせる
train_binary_dict = {'Date': train_dates, 'binary': y_train_pred_flat}
train_binary_df = pd.DataFrame(train_binary_dict)

# ValidationのDateとPredicted valueを組み合わせる
val_binary_dict = {'Date': val_dates, 'binary': y_validation_pred_flat}
val_binary_df = pd.DataFrame(val_binary_dict)

# TrainとValidationを結合して一つのdfを作成
attention_binary_df = pd.concat([train_binary_df, val_binary_df])

# Dateをインデックスとして設定
attention_binary_df.set_index('Date', inplace=True)

print(attention_binary_df)
