...
model.add(Dense(1, activation='sigmoid'))  # 出力層の活性化関数を'sigmoid'に

# 学習率を指定したAdamオプティマイザ
optimizer = Adam(learning_rate=0.0001)

# モデルのコンパイル
model.compile(optimizer=optimizer, loss='binary_crossentropy', metrics=['accuracy'])  # 損失関数を'binary_crossentropy'に
...
binary_df = pd.concat([train_binary_df, val_binary_df])

# Dateをインデックスとして設定
binary_df.set_index('Date', inplace=True)

print(binary_df)

# binary_dfをCSV形式で保存
binary_df.to_csv('YOURPATH')

#y_train_pred_label = (y_train_pred > 0.5).astype(int)
#y_val_pred_label = (y_val_pred > 0.5).astype(int)

test_pred = model.predict(X_test)
print(f"Predicted Closing Price for the Next Day: {test_pred[0][0]:.2f}") #9/29 0
